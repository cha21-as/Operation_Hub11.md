...
A missing/invalid `x-actor-role`, or a `staff` actor missing/invalid `x-actor-department`, is itself an invalid request — `400`, not a crash:
```json
{ "statusCode": 400, "message": "staff actors must send a valid x-actor-department header (IT, HR, or FINANCE)", "error": "Bad Request" }
```
...
With the backend running (`cd backend && npm run start`):

```bash
# Create a request (valid) — 201
curl -i -X POST http://localhost:3000/requests \
  -H "Content-Type: application/json" \
  -d '{"title":"Laptop wont turn on","department":"IT"}'
# copy the "id" from the response into $ID below

# Invalid request rejected on purpose — 400 (missing title)
curl -i -X POST http://localhost:3000/requests \
  -H "Content-Type: application/json" \
  -d '{"department":"IT"}'

# Invalid request rejected on purpose — 400 (unknown department)
curl -i -X POST http://localhost:3000/requests \
  -H "Content-Type: application/json" \
  -d '{"title":"Laptop wont turn on","department":"MARKETING"}'

# Authorization DENIED — the requester is not staff — 403
curl -i -X POST http://localhost:3000/requests/$ID/start \
  -H "x-actor-role: employee"

# Authorization DENIED — staff from a different department — 403
curl -i -X POST http://localhost:3000/requests/$ID/start \
  -H "x-actor-role: staff" -H "x-actor-department: HR"

# Expected failure handled on purpose — unknown id — 404
curl -i -X POST http://localhost:3000/requests/00000000-0000-0000-0000-000000000000/start \
  -H "x-actor-role: staff" -H "x-actor-department: IT"

# Authorization ALLOWED — matching department staff — 201
curl -i -X POST http://localhost:3000/requests/$ID/start \
  -H "x-actor-role: staff" -H "x-actor-department: IT"

# Regression — invariant still holds: cannot skip in_progress — 400
curl -i -X POST http://localhost:3000/requests/$ID/resolve \
  -H "x-actor-role: employee"
curl -i -X POST http://localhost:3000/requests/$ID/resolve \
  -H "x-actor-role: staff" -H "x-actor-department: HR"

# Move it forward for real, then confirm it's terminal — 201, then 400
curl -i -X POST http://localhost:3000/requests/$ID/resolve \
  -H "x-actor-role: staff" -H "x-actor-department: IT"
curl -i -X POST http://localhost:3000/requests/$ID/start \
  -H "x-actor-role: staff" -H "x-actor-department: IT"
...
curl -s http://localhost:3000/requests/$ID | python3 -m json.tool
```
